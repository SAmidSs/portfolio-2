$(function () {
    $('.slider__items').slick({
        infinite: false,
        dots: true,
        slidesToShow: 1,
        slidesToScroll: 1,
        prevArrow: '<button type="button" class="slick-prev"><svg width="34" height="8" viewBox="0 0 34 8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1.06588 3.64644C0.870617 3.84171 0.870617 4.15829 1.06588 4.35355L4.24786 7.53553C4.44312 7.73079 4.7597 7.73079 4.95497 7.53553C5.15023 7.34027 5.15023 7.02369 4.95497 6.82842L2.12654 4L4.95497 1.17157C5.15023 0.976308 5.15023 0.659726 4.95497 0.464464C4.7597 0.269201 4.44312 0.269201 4.24786 0.464464L1.06588 3.64644ZM33.4194 3.5L1.41943 3.5L1.41943 4.5L33.4194 4.5L33.4194 3.5Z" fill="black"/></svg>Prev</button >',

        nextArrow: '<button type="button" class="slick-next">Next<svg width="33" height="8" viewBox="0 0 33 8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M32.773 4.35355C32.9683 4.15829 32.9683 3.84171 32.773 3.64645L29.591 0.464466C29.3957 0.269204 29.0792 0.269204 28.8839 0.464466C28.6886 0.659728 28.6886 0.976311 28.8839 1.17157L31.7123 4L28.8839 6.82843C28.6886 7.02369 28.6886 7.34027 28.8839 7.53553C29.0792 7.7308 29.3957 7.7308 29.591 7.53553L32.773 4.35355ZM0.419434 4.5H32.4194V3.5H0.419434V4.5Z" fill="black"/></svg></button>'
    });


    $(".star").rateYo({
        rating: 5,
        starWidth: "24px",
        spacing: "8px",
        readOnly: true,
        ratedFill: '#FFC107',
        starSvg: '<svg width="20px" height="19px"viewBox="0 0 20 19"><path d="M9.25866 0.927052C9.55801 0.00574112 10.8614 0.00573993 11.1608 0.927051L12.6794 5.60081C12.8132 6.01284 13.1972 6.2918 13.6304 6.2918H18.5447C19.5134 6.2918 19.9162 7.53141 19.1325 8.10081L15.1568 10.9894C14.8063 11.244 14.6596 11.6954 14.7935 12.1074L16.3121 16.7812C16.6114 17.7025 15.557 18.4686 14.7732 17.8992L10.7975 15.0106C10.447 14.756 9.97242 14.756 9.62193 15.0106L5.64619 17.8992C4.86248 18.4686 3.808 17.7025 4.10735 16.7812L5.62595 12.1074C5.75982 11.6954 5.61316 11.244 5.26268 10.9894L1.28694 8.10081C0.503224 7.53141 0.905999 6.2918 1.87472 6.2918H6.78901C7.22223 6.2918 7.60619 6.01284 7.74006 5.60081L9.25866 0.927052Z" /></svg>'
    });



    $('.header__menu-btn').on('click', function(){
        $('.menu__list').toggleClass('menu__list--active');
        $('.header__menu-btn').toggleClass('header__menu-btn--active');
    });

});